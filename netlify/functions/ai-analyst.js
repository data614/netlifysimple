...
import {
  ...
  returnOnEquity,
  toQuantNumber,
} from '../../src/assets/utils/quant-math.js';
...
const formatValuationSection = (symbol, valuationData) => {
  ...
  return [
    ...
    `- Growth outlook: base ${fmtPercent(growth.base, true)} Â· bull ${fmtPercent(growth.bull, true)} Â· bear ${fmtPercent(growth.bear, true)}`,
    scenarios?.bull && scenarios?.bear
      ? `- Scenario targets: bull ${fmtCurrency(scenarios.bull)} Â· bear ${fmtCurrency(scenarios.bear)}`
      : null,
  ]
    .filter(Boolean)
    .join('\n');
