...
/* Formatting */
const fmt = (n) => (Number.isFinite(Number(n)) ? Number(n).toLocaleString(undefined, { maximumFractionDigits: 2 }) : 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â');
const fmtVol = (n) => (Number.isFinite(Number(n)) ? Number(n).toLocaleString() : 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â');
const fmtPct = (n) => {
  const num = Number(n);
  if (!Number.isFinite(num)) return 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
  const sign = num >= 0 ? '+' : '';
...
const eventFeedCache = createMonitoredCache('eventFeed', { ttl: 5 * 60 * 1000, maxEntries: 48 }); // Increased from 3min to 5min
...
function defaultTiingoCacheTtl(params = {}) {
  ...
  if (kind === 'intraday_latest') return 15_000; // Increased from 10s to 15s for live quotes
  if (kind === 'intraday') return 30_000; // Increased from 20s to 30s for intraday charts
  if (kind === 'eod') return 900_000; // Increased from 10min to 15min for daily data
  if (kind === 'valuation') return 30 * 60_000; // Increased from 15min to 30min for valuation
  if (kind === 'news') return 10 * 60_000; // 10min cache for news
  if (kind === 'filings') return 60 * 60_000; // 1hr cache for SEC filings
  if (kind === 'statements') return 45 * 60_000; // 45min cache for financial statements
  return 120_000; // Increased default from 60s to 2min
}
...
async function callTiingo(params, options = {}) {
  ...
  try {
    ...
    // ensure bad entries arenÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¾Ãƒâ€šÃ‚Â¢t retained
...
const WATCHLIST_REFRESH_INTERVAL = 30 * 1000; // Reduced from 60s to 30s for more responsive updates
...
const CLOCK_ZONES = [
  { label: 'New York (ET)', tz: 'America/New_York', marketStart: '09:30', marketEnd: '16:00' },
  { label: 'London (UK)', tz: 'Europe/London', marketStart: '08:00', marketEnd: '16:30' },
  { label: 'Tokyo (JST)', tz: 'Asia/Tokyo', marketStart: '09:00', marketEnd: '15:00' },
  { label: 'Sydney (AEST)', tz: 'Australia/Sydney', marketStart: '10:00', marketEnd: '16:00' },
];

// Function to check if a market is currently open
function isMarketOpen(timezone, marketStart, marketEnd) {
  const now = new Date();
  ...
  const [startHour, startMinute] = marketStart.split(':').map(Number);
  const [endHour, endMinute] = marketEnd.split(':').map(Number);
...
const SEARCH_PLACEHOLDER = 'Start typing to searchÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦';
...
function renderWatchlistNow() {
  ...
  watchlist.forEach((item) => {
    ...
    const price = stats ? fmt(stats.price) : 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
    const pct = stats ? fmtPct(stats.changePct) : 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
    ...
    const cleanExchange = exchange.startsWith('X') ? exchange.substring(1) : exchange;
    const displaySymbol = cleanExchange ? `${item.symbol}:${cleanExchange}` : item.symbol;
...
function updateApiBadge(meta = {}) {
  ...
  badge.title = meta.chosenKey
    ...
    : preview
      ...
      : 'Tiingo token not configured ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â using fallback data';
...
function updateChartStatus(meta = {}, warning = '', count = 0) {
  ...
  if (source === 'live') {
    ...
    parts.push('Awaiting market dataÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦');
    css += ' warning';
  }
  if (count) {
  ...
  el.textContent = parts.join(' ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· ') || 'Awaiting market dataÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦';
  el.className = css;
...
async function refreshWatchlistQuotes() {
  ...
  const results = await Promise.all(entries.map((item) =>
    callTiingo({ symbol: item.symbol, kind: 'intraday_latest' }, { silent: true, cacheTtl: 20_000 }) // Increased cache for watchlist quotes
      .then((res) => ({ item, res }))
      .catch((err) => {
...
function startWatchlistAutoRefresh() {
  ...
  watchlistRefreshTimer = setInterval(() => {
    refreshWatchlistQuotes();
    // Smart current symbol refresh - only if not recently cached
    if (currentSymbol) {
      const key = `symbol=${currentSymbol}&kind=intraday_latest`;
      const cached = tiingoRequestCache.get(key);
...
/**
 * High-performance ticker update - only refreshes price data without full chart reload
 * @param {string} symbol - The ticker symbol to update
 * @param {boolean} forceUpdate - Force update even if cached
 */
async function fastTickerUpdate(symbol, forceUpdate = false) {
  if (!symbol) return;
  
  try {
    const options = { silent: true };
    if (forceUpdate) options.forceRefresh = true;
    
    // Only update the latest quote, not charts or other heavy data
    const result = await callTiingo({ symbol, kind: 'intraday_latest' }, options);
    
    if (result?.data?.[0]) {
      const latestData = result.data[0];
      
      // Update just the price display elements without full page render
      const priceElement = $('currentPrice');
      const changeElement = $('priceChange');
      const changePercentElement = $('priceChangePercent');
      
      if (priceElement) {
        priceElement.textContent = fmt(latestData.close || latestData.last);
      }
      
      if (latestData.close && latestData.prevClose) {
        const change = latestData.close - latestData.prevClose;
        const changePercent = (change / latestData.prevClose) * 100;
        
        if (changeElement) {
          changeElement.textContent = (change >= 0 ? '+' : '') + fmt(change);
          changeElement.className = change >= 0 ? 'positive' : 'negative';
        }
        
        if (changePercentElement) {
          changePercentElement.textContent = (change >= 0 ? '+' : '') + changePercent.toFixed(2) + '%';
          changePercentElement.className = change >= 0 ? 'positive' : 'negative';
        }
      }
      
      // Update last refresh time
      if (result.meta) updateApiBadge(result.meta);
    }
  } catch (err) {
    console.warn('Fast ticker update failed for', symbol, err);
  }
}
...
function startClock() {
  ...
  const items = CLOCK_ZONES.map(({ label, tz, marketStart, marketEnd }) => {
    ...
    return { wrapper, timeEl, dateEl, tz, marketStart, marketEnd };
  });

  const renderTick = () => {
    const now = Date.now();
    items.forEach(({ wrapper, timeEl, dateEl, tz, marketStart, marketEnd }) => {
      ...
      // Add or remove market-live class based on market status
      const isLive = isMarketOpen(tz, marketStart, marketEnd);
      if (isLive) {
        wrapper.classList.add('market-live');
      } else {
        wrapper.classList.remove('market-live');
      }
    });
  };
  ...
  scheduleNewsRender(() => {
    ...
    articles.forEach((article) => {
      ...
      item.innerHTML = `
        ...
        <small>${metaParts.join(' ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· ')}</small>
      `;
...
async function loadNews(source = 'All') {
  ...
  if (!feed) return;
  const qTokens = [];
  if (currentSymbol) qTokens.push(currentSymbol);
  if (currentSymbolName) qTokens.push(currentSymbolName);
  const q = qTokens.join(' OR ');
  const cacheKey = q ? `news:${source}:${q}` : `news:${source}`;
  ...
  feed.innerHTML = '<div class="muted">Loading newsÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦</div>';

  try {
    const { cancelled, result: payload } = await runLatestNewsTask(async () => {
      const response = await newsRequestCache.resolve(cacheKey, async () => {
        const resp = await fetch(`${API}/news?${new URLSearchParams(Object.assign({ source }, q ? { q } : {})).toString()}`, {
...
function mapDividendEvent(dividend, symbol) {
  ...
  return {
    ...
    title,
    summary: '',
    details: detailsParts.join(' ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· '),
    url: '',
    timestamp: iso,
    timeValue: ms,
...
function summariseEventStatus(sources = [], warnings = [], errors = [], count = 0) {
  ...
  let text = 'Events: ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
  if (hasMock) {
    text = 'Events: Sample';
  } else if (hasFallback) {
  ...
  return {
    text,
    className,
    title: titleParts.join(' ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ '),
...
function updateEventFeedBadge(status = {}) {
  ...
  scheduleEventFeedRender(() => {
    badge.textContent = status.text || 'Events: ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
...
function renderEventFeed(container, items = [], status = {}) {
  scheduleEventFeedRender(() => {
    ...
    if (alerts.length) {
      ...
      warningEl.textContent = alerts.join(' ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ ');
    ...
    validItems.forEach((event) => {
      ...
      if (event.timestamp) {
        ...
        if (absolute) {
          metaParts.push(relative ? `${absolute} ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· ${relative}` : absolute);
        }
      }
      if (metaParts.length) {
        const meta = document.createElement('span');
        meta.textContent = metaParts.join(' ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· ');
...
async function loadEventFeed(symbol) {
  ...
  updateEventFeedBadge({ text: 'Events: LoadingÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦', className: 'chip' });
  scheduleEventFeedRender(() => {
    container.innerHTML = '<div class="event-feed-empty">Loading eventsÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦</div>';
  });
...
async function sendWatchlistSummary() {
  ...
  statusEl.textContent = 'Sending summaryÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦';
...
function setSearchLoading() {
  ...
  scheduleSearchRender(() => {
    if (!searchResultsEl) return;
    searchResultsEl.innerHTML = '<div class="search-loading">SearchingÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦</div>';
  });
}
...
function renderSearchResults(items) {
  ...
  scheduleSearchRender(() => {
    ...
    items.forEach((item) => {
      ...
      const meta = [type, country].filter(Boolean).join(' ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ ');
      ...
      row.innerHTML = `
        <div class="search-result-main">
          ...
          <div class="search-result-text">
            <div class="search-result-symbol">${displaySymbol}</div>
...
async function performSearch(query) {
  const q = query.trim();
  if (q.length < 2) {
    clearSearchResults('Keep typing to searchÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¦');
    return;
  }
...
function setupSearch() {
  ...
  if (searchInputEl) {
    ...
    searchInputEl.addEventListener('keydown', (event) => {
      ...
      const results = Array.from(searchResultsEl.querySelectorAll('.search-result-item'));
      if (!results.length) return;
      
      const focused = searchResultsEl.querySelector('.search-result-item.keyboard-focused');
      let currentIndex = focused ? results.indexOf(focused) : -1;
      
      if (event.key === 'ArrowDown') {
        event.preventDefault();
        if (focused) focused.classList.remove('keyboard-focused');
        currentIndex = (currentIndex + 1) % results.length;
        results[currentIndex].classList.add('keyboard-focused');
        ...
        event.preventDefault();
        const loadBtn = focused.querySelector('.search-result-btn.load');
...
function updateRangeStats(rows) {
  ...
  if (!highs.length || !lows.length) {
    $('stat52wHigh').textContent = 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
    $('stat52wLow').textContent = 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
    return;
  }
    ...
    if (statementsRes.status === 'fulfilled') {
      ...
      incomeRows.forEach((row, index) => {
        ...
        events.push({
          ...
          description: details.join(' ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· '),
          style: meta,
        });
      });
    }
    ...
    if (actionsRes.status === 'fulfilled') {
      ...
      dividends.forEach((dividend, index) => {
        ...
        events.push({
          ...
          date: dateKey,
          title: meta.label,
          description: Number.isFinite(amount) ? `Ex-Date ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ ${fmt(amount)}` : 'Ex-Date',
          style: meta,
        });
      });
...
function renderQuote(q) {
  if (!q) {
    $('stockPrice').textContent = 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
    $('stockChange').textContent = 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
    $('stockChange').className = 'stock-change';
    $('statOpen').textContent = 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
    $('statHigh').textContent = 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
    $('statLow').textContent = 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
    $('statVolume').textContent = 'ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â';
    return;
  }
...
function renderChart(rows, intraday, events = []) {
  ...
  const eventSeries = dateKeys.map((key, index) => {
    ...
    return {
      ...
      events: eventList,
      style,
      label: eventList.map((evt) => evt.title).join(' ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ '),
    };
  });
...
async function loadSymbol(symbol, name, exchange) {
  ...
  try {
    await loadNews(preferredNewsSource);
  } catch (err) {
