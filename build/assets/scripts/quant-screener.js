...
function renderHeatmap(rows) {
  ...
  limited.forEach((row, index) => {
    ...
    const rank = index + 1;
    const symbolLabel = sanitizeText(row.symbol || 'â€”') || 'â€”';
...
function renderTable(rows) {
  ...
  rows.forEach((row) => {
    ...
    const safeSymbol = escapeHtml(row.symbol || 'â€”');
    const safeSector = escapeHtml(row.sector || 'â€”');
    ...
    const safeSummary = escapeHtml(row.summary || 'â€”');
    tr.innerHTML = `
      <td>${safeSymbol}</td>
      <td>${safeSector}</td>
      <td>${safeMarketCap}</td>
...
function updateSummary(rows) {
  ...
  chip.textContent = `${rows.length} matches Â· Avg upside ${fmtPercent(avgUpsideDisplay)}`;
  publishRuntimeBridge();
}
...
async function runScreen() {
  ...
  renderHeatmap([]);
  updateSummary([]);
  setStatus(`Screening ${universe.length} tickers using ChatGPTâ€‘5â€¦`, 'info');
